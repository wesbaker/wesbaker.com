// Adds a copy-to-clipboard button and a language label to every code block.
...
document.addEventListener("DOMContentLoaded", function () {
  // Keep the button and label pinned to the visible edge of code blocks that
  // scroll horizontally.
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        if (clipboardBtn) {
          clipboardBtn.style.right = entry.isIntersecting
            ? "5px"
            : `-${entry.boundingClientRect.right - pre.clientWidth + 5}px`;
        }

        if (label) {
          label.style.right = entry.isIntersecting
            ? "0px"
            : `-${entry.boundingClientRect.right - pre.clientWidth}px`;
        }
      });
    },
  ...
  document.querySelectorAll("pre > code").forEach((codeBlock) => {
    ...
    // Shiki records the fence's language on the `pre` element.
